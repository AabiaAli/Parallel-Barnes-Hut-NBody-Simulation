/*
 * The parallel version of the N-Body problem with Gravitational Softening
 * 
 * Optimization: Added softening length EPSILON to prevent numerical singularities
 * when particles get very close to each other.
 * 
 * Author: Dileban Karunamoorthy (dileban@gmail.com)
 * Modified: Added softening constant
 */
...
/* GRAVITATIONAL SOFTENING - Prevents numerical singularities */
#define EPSILON 1000.0    /* Softening length in meters. 
                             Small relative to 1e6 m domain (0.1%).
                             Forces are smoothed when particles get closer
                             than ~1000m, preventing infinite forces. */
...
/*
 * Computes the SOFTENED distance between two particles in space
 * 
 * OPTIMIZATION: Added EPSILON² term to prevent division by zero and
 * bound the maximum force when particles get extremely close.
 * 
 * Without softening: d = sqrt(dx² + dy² + dz²)
 * With softening:    d = sqrt(dx² + dy² + dz² + EPSILON²)
 * 
 * This ensures d > 0 always, so force = G*m1*m2/d² is always finite.
 */
double compute_distance(Position a, Position b){
    ...
    /* SOFTENED DISTANCE - the only change needed for this optimization */
   ...
   for (i = 0; i < part_size; i++) {
      ...
      for (j = 0; j < N; j++){
         ...
         double f = (G * (mass[i + pindex] * mass[j]) / 
                         (d * d));  // Note: d is already softened
